...
import { type LucideIcon, Home, Menu, Package, ShoppingCart } from "lucide-react";
      ...
      {open && (
        <div className="fixed top-14 left-4 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 min-w-44">
          {navItems.map(({ href, label, icon: Icon, onClick, badge }) => {
            ...
            const cartBadge = isCart && count > 0 ? count : null;
            const shownBadge = badge ?? cartBadge;
            const cls = "flex items-center gap-3 px-4 py-3.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 w-full text-left";
