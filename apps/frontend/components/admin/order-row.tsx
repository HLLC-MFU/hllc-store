...
export function OrderRow({ order, onStatusChange, onApproveSlip, onSaveTracking, onCancel, t, onViewSlip, useModal = false, onModalOpen, onModalClose }: {
  ...
  const slipScrollRefDesktop = React.useRef<HTMLDivElement>(null);
                ...
                {showCancel && (
                  <div className="mt-3 flex flex-col gap-2.5 animate-in slide-in-from-top-2 duration-150">
                    <textarea
                      ...
                      rows={2}
                      className="w-full text-xs text-gray-900 bg-red-50/50 border border-red-200 rounded-xl px-3 py-2.5 resize-none outline-none focus:border-red-400/60 focus:ring-2 focus:ring-red-400/10 placeholder:text-red-400 transition-all"
                    />
                    <Button
                      onClick={() => {
    ...
    {useModal && open && typeof window !== "undefined" && createPortal(
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 lg:p-8 bg-black/80 animate-in fade-in duration-150" style={{zIndex:50}} onClick={() => toggleOpen(false)}>
        <div className={`bg-white rounded-t-3xl sm:rounded-3xl w-full overflow-hidden shadow-2xl animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-200 flex ${hasSlip ? "flex-col lg:flex-row max-w-lg lg:max-w-4xl max-h-[92dvh] sm:max-h-[90vh] lg:max-h-[88vh]" : "flex-col max-w-lg max-h-[92dvh] sm:max-h-[90vh]"}`} onClick={(e) => e.stopPropagation()}>
          ...
          {hasSlip && (
            <div className="hidden lg:flex flex-col w-[42%] shrink-0 border-r border-gray-100 bg-gray-50/60">
              ...
              <div className="relative flex-1 bg-gray-100">
                ...
                >
                  {slipImages.map((src, i) => (
                    <button key={`d-${src}-${i}`} type="button" onClick={() => onViewSlip(slipImages, i)}
                      className="w-full h-full shrink-0 snap-center relative group cursor-pointer block">
                      <Image fill src={src} alt={`slip ${i + 1}`} className="object-contain p-2" sizes="45vw" />
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/15 flex items-end px-4 pb-4 transition-colors">
                        <span className="opacity-0 group-hover:opacity-100 text-white text-xs font-black bg-black/60 px-3 py-1.5 rounded-xl">ขยาย</span>
                      </div>
                    </button>
                  ))}
                </div>
                {slipImages.length > 1 && (
                  <>
                    {slipIndex > 0 && (
                      <button type="button" onClick={() => { const n = slipIndex - 1; slipScrollRefDesktop.current?.scrollTo({ left: n * slipScrollRefDesktop.current.clientWidth, behavior: "smooth" }); setSlipIndex(n); }}
                        className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 backdrop-blur-sm shadow-md flex items-center justify-center text-gray-700 hover:bg-white active:scale-90 transition-all">
                        <ChevronLeft className="w-5 h-5" />
                      </button>
                    )}
                    {slipIndex < slipImages.length - 1 && (
                      <button type="button" onClick={() => { const n = slipIndex + 1; slipScrollRefDesktop.current?.scrollTo({ left: n * slipScrollRefDesktop.current.clientWidth, behavior: "smooth" }); setSlipIndex(n); }}
                        className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white/80 backdrop-blur-sm shadow-md flex items-center justify-center text-gray-700 hover:bg-white active:scale-90 transition-all">
                        <ChevronRight className="w-5 h-5" />
                      </button>
                    )}
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2">
                      {slipImages.map((_, i) => (
                        <button key={i} type="button" onClick={() => { slipScrollRefDesktop.current?.scrollTo({ left: i * slipScrollRefDesktop.current.clientWidth, behavior: "smooth" }); setSlipIndex(i); }}
                          className={`rounded-full transition-all ${i === slipIndex ? "w-5 h-2 bg-white shadow" : "w-2 h-2 bg-white/60"}`} />
                      ))}
                    </div>
                  </>
                )}
              ...
              {slipIndex > 0 && previousSlips[slipIndex - 1]?.reviewNote && (
                <div className="px-5 py-3 bg-red-50/60 border-t border-red-100 shrink-0">
                  <p className="text-[11px] font-semibold text-red-600">{previousSlips[slipIndex - 1].reviewNote}</p>
                </div>
              )}
            </div>
          )}
                  ...
                  return (
                    <div key={s} className="flex flex-col items-center z-10">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all shadow-sm ${
                          ...
                          : active && isPickup
                          ? "bg-white border-emerald-500 text-emerald-500 scale-110 ring-4 ring-emerald-500/5"
                          : active
                          ? "bg-white border-brand text-brand scale-110 ring-4 ring-brand/5"
          ...
          {/* Slip — mobile/tablet only; desktop shows in left column */}
          {hasSlip && (
            ...
            {showCancel && (
              <div className="flex flex-col gap-2 animate-in slide-in-from-bottom-2 duration-150 border-t border-red-100 pt-3">
                ...
                <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} placeholder="ระบุเหตุผลการยกเลิก..." rows={2}
                  className="w-full text-xs text-gray-900 bg-red-50/50 border border-red-200 rounded-xl px-3 py-2.5 resize-none outline-none placeholder:text-red-400 transition-all" />
                <div className="grid grid-cols-2 gap-2">
